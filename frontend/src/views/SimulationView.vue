      ...
      <div class="header-center">
        <div class="view-switcher">
          <button 
            ...
            @click="viewMode = mode"
          >
            {{ { graph: 'Граф', split: 'Сплит', workbench: 'Панель' }[mode] }}
          </button>
        </div>
      </div>
...
const handleNextStep = (params = {}) => {
  ...
  // 记录模拟轮数配置
  if (params.maxRounds) {
    addLog(`Пользовательское кол-во раундов: ${params.maxRounds}`)
  } else {
    addLog('Используются автоматические настройки раундов')
  }
...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('Обнаружена работающая симуляция, закрытие...')
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          addLog(`Ошибка закрытия среды: ${closeRes.error || 'неизвестная ошибка'}`)
          // 如果优雅关闭失败，尝试强制停止
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Исключение при закрытии среды: ${closeErr.message}`)
        // 如果优雅关闭异常，尝试强制停止
        await forceStopSimulation()
      }
    } else {
      ...
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('Обнаружена активная симуляция, остановка...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
...
const forceStopSimulation = async () => {
  try {
    ...
    if (stopRes.success) {
      ...
      addLog(`Ошибка принудительной остановки: ${stopRes.error || 'неизвестная ошибка'}`)
    }
  } catch (err) {
...
const loadSimulationData = async () => {
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      addLog(`Ошибка загрузки данных: ${simRes.error || 'неизвестная ошибка'}`)
    }
  } catch (err) {
