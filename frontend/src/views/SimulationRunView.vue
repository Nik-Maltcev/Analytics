...
const handleGoBack = async () => {
  ...
  addLog('Возврат к Step 2, закрытие симуляции...')
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Ошибка закрытия среды, принудительная остановка...`)
        try {
    ...
    addLog(`Ошибка проверки статуса: ${err.message}`)
  }
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          ...
          addLog(`Настройка времени: ${minutesPerRound.value} мин/раунд`)
        }
      } catch (configErr) {
        addLog(`Ошибка получения настроек, по умолчанию: ${minutesPerRound.value} мин/раунд`)
      }
...
const startGraphRefresh = () => {
  ...
  addLog('Автообновление графа включено (30с)')
  // 立即刷新一次，然后每30秒刷新
