...
const doStartSimulation = async () => {
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`Макс. кол-во раундов: ${props.maxRounds}`)
    }
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Старые логи очищены, симуляция перезапущена')
      }
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Все платформы завершили симуляцию')
        }
...
const handleNextStep = async () => {
  ...
  try {
    ...
    addLog(`✗ Исключение при запуске генерации: ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
onMounted(() => {
  addLog('Step3 инициализация симуляции')
  if (props.simulationId) {
    doStartSimulation()
  }
})
