              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">若首次运行，强烈建议切换至‘Настроить模式’减少模拟р数，以便快速预览效果并降低报错风险 ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
// State
const phase = ref(0) // 0: 初始化, 1: Генерация профилей, 2: 生成配置, 3: 完成
...
watch(currentStage, (newStage) => {
  if (newStage === '生成Agentпрофилей' || newStage === 'generating_profiles') {
    phase.value = 1
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    // 用户选择保持自动生成的р数，不传递 max_rounds 参数
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 输出 Profile 生成进度日志（仅当Кол-во:变化时）
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          addLog('正在生成Agentпрофилей配置...')
        } else if (data.generation_stage === 'generating_config') {
          addLog('正在调用LLMГенерация конфигурации симуляции参数...')
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.config.time_config) {
          ...
          addLog(`时间配置: 每р${tc.minutes_per_round}мин, 共${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}р`)
        }
...
const loadPreparedData = async () => {
  ...
  addLog(`已加载 ${profiles.value.length} штAgentпрофилей`)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // 显示详细配置摘要
        if (res.data.summary) {
          addLog(`  ├─ AgentКол-во:: ${res.data.summary.total_agents}шт`)
