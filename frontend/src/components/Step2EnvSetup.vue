    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- 模拟р数配置 - 只有在配置生成完成且р数计算出来后才显示 -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">模拟р数设定</span>
                <span class="section-desc">MiroFish автоматически планирует моделирование <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> ч，每р代表现实 <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> мин时间流逝</span>
              </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">шт体链接与交互图谱</span>
              </div>
            </div>
...
const customMaxRounds = ref(40)   // 默认推荐40р
...
watch(currentStage, (newStage) => {
  if (newStage === '生成Agent人设' || newStage === 'generating_profiles') {
    ...
    // 进入配置生成阶段，开始р询配置
    if (!configTimer) {
      addLog('开始Генерация конфигурации двух платформ...')
      startConfigPolling()
    }
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(`开始模拟，Настроитьр数: ${customMaxRounds.value} р`)
  } else {
    // 用户选择保持自动生成的р数，不传递 max_rounds 参数
    addLog(`开始模拟，使用自动配置р数: ${autoGeneratedRounds.value} р`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        addLog(`→ Agent人设 ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || 'Неизвестная профессия'})`)
        
        // 如果全部生成完成
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ 全部 ${currentCount} штAgent人设生成完成`)
        }
      }
    }
  } catch (err) {
...
const loadPreparedData = async () => {
  ...
  addLog(`已加载 ${profiles.value.length} штAgent人设`)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // 显示详细配置摘要
        if (res.data.summary) {
          addLog(`  ├─ Agent数量: ${res.data.summary.total_agents}шт`)
