    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Создание экземпляра симуляции, загрузка шаблонов параметров
          </p>
      ...
      <!-- Step 02: Генерация профилей агентов -->
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">02</span>
            <span class="step-title">Генерация профилей агентов</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Ожидаемое кол-во агентов</span>
            </div>
            <div class="stat-card">
          ...
          <div v-if="profiles.length > 0" class="profiles-preview">
            <div class="preview-header">
              <span class="preview-title">Сгенерированные профили агентов</span>
            </div>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <div class="config-item">
                  <span class="config-item-label">Время раунда</span>
                ...
                <div class="config-item">
                  <span class="config-item-label">Активных/ч</span>
                ...
                >
                  ...
                  <div class="agent-params">
                    ...
                    <div class="param-group">
                      ...
                      <div class="param-item">
                        <span class="param-label">Эмоц. уклон</span>
              ...
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  ...
                  <div class="platform-params">
                    ...
                    <div class="param-row">
                      <span class="param-label">Вирусный порог</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">Среда симуляции готова к запуску</p>
          
          <!-- Настройка раундов - показывать только после генерации конфигурации -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Настройка раундов</span>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">При первом запуске рекомендуем переключиться в режим 'Настроить' и уменьшить количество раундов для быстрого предпросмотра ➝</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            ...
            <div class="info-item">
              ...
              <span class="info-value">{{ { male: 'Муж', female: 'Жен', other: 'Другой' }[selectedProfile.gender] || selectedProfile.gender }}</span>
            </div>
            <div class="info-item">
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            <span class="section-label">Подробная предыстория</span>
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Опыт и стиль поведения</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Уникальные воспоминания</span>
                <span class="dim-desc">Память на основе реальных данных</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Социальные связи</span>
                <span class="dim-desc">штСвязи и интерактивный граф</span>
              </div>
            </div>
...
const props = defineProps({
  simulationId: String,  // Передано из родительского компонента
  projectData: Object,
  graphData: Object,
  systemLogs: Array
})
...
// State
const phase = ref(0) // 0: Инициализация, 1: Генерация профилей, 2: Генерация配置, 3: 完成
...
// раундов配置
const useCustomRounds = ref(false) // 默认使用自动配置р数
const customMaxRounds = ref(40)   // Рекомендуемое значение40р

// Watch stage to update phase
watch(currentStage, (newStage) => {
  if (newStage === 'ГенерацияAgentпрофилей' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === 'Генерация конфигурации симуляции' || newStage === 'generating_config') {
    phase.value = 2
    // 进入配置Генерация阶段，Началор询配置
    if (!configTimer) {
      addLog('НачалоГенерация конфигурации двух платформ...')
      startConfigPolling()
    }
...
// 从配置中计算自动Генерация的р数（不使用硬编码默认值）
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Возврат, если конфигурация не сгенерирована null
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // Возврат, если данные конфигурации неполные null
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // 确保最大р数不小于40（推荐值），避免滑动条范围异常
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // 用户Настроитьр数，传递 max_rounds 参数
    params.maxRounds = customMaxRounds.value
    addLog(`Начало симуляции, пользовательских раундов: ${customMaxRounds.value} р`)
  } else {
    // 用户选择保持自动Генерация的р数，不传递 max_rounds 参数
    addLog(`Начало симуляции, автоматическая настройка раундов: ${autoGeneratedRounds.value} р`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Обнаружена завершённая подготовка, используем')
      ...
      // 立即设置Ожидаемое кол-во агентов（从prepare接口返回值获取）
      ...
      addLog('НачалорОпрос прогресса подготовки...')
      // Началор询进度
      startPolling()
      // Начало实时获取 Profiles
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    console.warn('рОшибка опроса статуса::', err)
  }
}
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 输出 Profile Генерация进度日志（仅当Кол-во:变化时）
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (currentCount === 1) {
          addLog(`НачалоГенерацияAgentпрофилей...`)
        }
        addLog(`→ Agentпрофилей ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || 'Неизвестная профессия'})`)
    ...
    console.warn('Ошибка загрузки профилей:', err)
  }
}
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 输出配置Генерация阶段日志（避免重复）
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        lastLoggedConfigStage = data.generation_stage
        if (data.generation_stage === 'generating_profiles') {
          addLog('Генерация профилей агентов...')
      ...
      if (data.config_generated && data.config) {
        ...
        // 显示详细配置摘要
        if (data.summary) {
          addLog(`  ├─ AgentКол-во:: ${data.summary.total_agents}шт`)
          addLog(`  ├─ Длительность: ${data.summary.simulation_hours}ч`)
        ...
        if (data.config.time_config) {
          ...
          addLog(`Время: каждый раунд ${tc.minutes_per_round}мин,  всего ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}р`)
        }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // 显示详细配置摘要
        if (res.data.summary) {
          addLog(`  ├─ Кол-во агентов: ${res.data.summary.total_agents}`)
          addLog(`  ├─ Длительность: ${res.data.summary.simulation_hours} ч`)
          addLog(`  └─ Начальные посты: ${res.data.summary.initial_posts_count}`)
        }
        ...
        // Конфигурация ещё не сгенерирована, начинаем опрос
        addLog('Генерация конфигурации, ожидание...')
        startConfigPolling()
      }
    }
  } catch (err) {
...
onMounted(() => {
  // 自动Начало准备流程
  if (props.simulationId) {
    addLog('Step2 Инициализация среды')
