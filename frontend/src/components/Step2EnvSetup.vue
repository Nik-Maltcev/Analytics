              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">При первом запуске рекомендуем уменьшить количество раундов，‘Настроить模式’减少раундов，以便快速预览效果并降低报错风险 ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(`Начало模拟，Настроитьр数: ${customMaxRounds.value} р`)
  } else {
    // 用户选择保持自动Генерация的р数，不传递 max_rounds 参数
    addLog(`Начало模拟，使用自动配置р数: ${autoGeneratedRounds.value} р`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.expected_entities_count) {
        ...
        addLog(`从Zep图谱读取到 ${res.data.expected_entities_count} шт实体`)
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ 全部 ${currentCount} штAgentпрофилейГенерация完成`)
        }
      }
    }
  } catch (err) {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.config.time_config) {
          ...
          addLog(`时间配置: 每р${tc.minutes_per_round}мин,  всего ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}р`)
        }
