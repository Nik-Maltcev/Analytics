...
class ReportLogger:
    ...
    def log_start(self, simulation_id: str, graph_id: str, simulation_requirement: str):
        ...
        self.log(
            ...
            details={
                ...
                "message": "Начало генерации отчёта"
            }
        )
    ...
    def log_planning_context(self, context: Dict[str, Any]):
        ...
        self.log(
            ...
            stage="planning",
            details={
                "message": "Получение контекста симуляции",
                "context": context
            }
        )
    ...
    def log_llm_response(
        ...
        self.log(
            ...
            details={
                ...
                "message": f"LLM ответ (инструменты: {has_tool_calls}, финальный ответ: {has_final_answer})"
            }
        )
    ...
    def log_section_content(
        ...
        self.log(
            ...
            details={
                ...
                "message": f"Раздел {section_title} — контент сгенерирован"
            }
        )
    ...
    def log_section_full_complete(
        ...
        self.log(
            ...
            details={
                ...
                "message": f"Раздел {section_title} завершён"
            }
        )
...
【Глубокий анализ — мощный инструмент поиска】
Мощная функция поиска для глубокого анализа:
1. Автоматически разбивает вопрос на подвопросы
2. Ищет информацию в графе симуляции по нескольким измерениям
3. Объединяет семантический поиск, анализ сущностей и отслеживание связей
4. Возвращает наиболее полные и глубокие результаты

【Сценарии использования】
- Глубокий анализ темы
- Изучение нескольких аспектов события
- Получение материалов для разделов отчёта

【Возвращает】
- Оригинальные факты (можно цитировать)
- Аналитика ключевых сущностей
- Анализ цепочек связей"""

TOOL_DESC_PANORAMA_SEARCH = """\
【Панорамный поиск — полная картина】
Инструмент для получения полной картины результатов симуляции:
1. Получает все связанные узлы и отношения
2. Разделяет текущие и исторические/устаревшие факты
3. Помогает понять эволюцию событий

【Сценарии использования】
- Полная хронология развития событий
- Сравнение изменений на разных этапах
- Полная информация о сущностях и связях

【Возвращает】
- Текущие активные факты (последние результаты)
- Исторические/устаревшие факты (история изменений)
- Все связанные сущности"""

TOOL_DESC_QUICK_SEARCH = """\
【Быстрый поиск】
Лёгкий инструмент для быстрого поиска информации.
...
【Глубинное интервью — реальный опрос агентов (две платформы)】
Вызов API интервью OASIS для реального опроса агентов симуляции!
Это не имитация LLM, а реальные ответы агентов через API.
По умолчанию интервью на обеих платформах (Twitter и Reddit).

Процесс:
1. Автоматически читает профили всех агентов
2. Интеллектуально выбирает наиболее релевантных агентов
3. Автоматически генерирует вопросы
4. Проводит реальное интервью на двух платформах
5. Объединяет результаты, предоставляя многоракурсный анализ

【Сценарии использования】
- Мнения разных ролей о событии
- Сбор мнений и позиций
- Реальные ответы агентов из OASIS
- Живые цитаты для отчёта

【Возвращает】
- Информация о респондентах
- Ответы агентов на обеих платформах
- Ключевые цитаты (можно использовать напрямую)
- Резюме интервью и сравнение позиций

【Важно】Требуется работающая среда OASIS!"""

# ── 大纲规划 prompt ──

PLAN_SYSTEM_PROMPT = """\
ВАЖНО: Весь ваш ответ, включая title, summary и все section titles, ДОЛЖЕН быть на РУССКОМ языке. Никакого китайского или английского текста.
...
【Ваша задача】
Написать «прогнозный отчёт», отвечающий на вопросы:
1. Что произошло в будущем при заданных условиях?
2. Как реагировали и действовали различные группы агентов?
3. Какие тенденции и риски выявила симуляция?

【Позиционирование отчёта】
- ✅ Это прогнозный отчёт на основе симуляции: «что будет, если...»
- ✅ Фокус на результатах: развитие событий, реакции групп, эмерджентные явления, риски
- ✅ Поведение агентов — это прогноз поведения людей в будущем
- ❌ Это НЕ анализ текущей реальности
- ❌ Это НЕ общий обзор общественного мнения

【Ограничения по разделам】
- Минимум 2, максимум 5 разделов
- Без подразделов, каждый раздел — цельный текст
- Содержание должно быть ёмким, сфокусированным на ключевых находках
- Структура разделов определяется вами на основе результатов

Выведите план отчёта в формате JSON:
{
    "title": "Заголовок отчёта",
    "summary": "Резюме отчёта (одно предложение о ключевых находках)",
    "sections": [
        {
...
Переменные, внесённые в мир симуляции (условия моделирования): {simulation_requirement}
...
【Примеры прогнозируемых фактов из симуляции】
{related_facts_json}

Рассмотрите эту репетицию будущего «взглядом бога»:
1. Какое состояние будущего проявилось при заданных условиях?
2. Как реагировали и действовали различные группы (агенты)?
3. Какие тенденции будущего выявила симуляция?

На основе результатов прогноза разработайте оптимальную структуру разделов отчёта.
...
ВАЖНО: Весь текст раздела ДОЛЖЕН быть на РУССКОМ языке. Переводите любой контент с других языков на русский.

Вы эксперт по написанию «прогнозных отчётов», сейчас пишете один из разделов.

Заголовок отчёта: {report_title}
Резюме отчёта: {report_summary}
Сценарий прогноза (условия моделирования): {simulation_requirement}
...
Мир симуляции — репетиция будущего. Мы внесли определённые условия,
поведение агентов — это прогноз поведения людей в будущем.

Ваша задача:
- Раскрыть, что произошло в будущем при заданных условиях
- Показать реакции и действия различных групп (агентов)
- Выявить тенденции, риски и возможности

❌ Не пишите анализ текущей реальности
✅ Фокусируйтесь на «что будет» — результаты симуляции и есть прогноз

═══════════════════════════════════════════════════════════════
【Главные правила — обязательны к соблюдению】
═══════════════════════════════════════════════════════════════

1. 【Обязательно используйте инструменты для наблюдения за миром симуляции】
   - Вы наблюдаете за репетицией будущего «взглядом бога»
   - Всё содержание должно основываться на событиях и поведении агентов
   - Запрещено использовать собственные знания для написания отчёта
   - Каждый раздел: минимум 3 вызова инструментов (максимум 5)

2. 【Обязательно цитируйте оригинальные высказывания агентов】
   - Высказывания агентов — прогноз поведения людей
   - Используйте формат цитат в отчёте:
     > «Определённая группа заявит: оригинальный текст...»
   - Эти цитаты — ключевые доказательства прогноза

3. 【Языковая согласованность — весь текст на русском языке】
   - Инструменты могут возвращать текст на английском или смешанном языке
   - Отчёт должен быть полностью на русском языке
   - При цитировании переводите всё на русский язык
   - Сохраняйте смысл, обеспечивая естественность изложения
   - Это правило применяется и к основному тексту, и к цитатам (формат >)

4. 【Достоверное представление результатов прогноза】
   - Содержание должно отражать результаты симуляции
   - Не добавляйте информацию, отсутствующую в симуляции
   - При недостатке информации — укажите это

═══════════════════════════════════════════════════════════════
【⚠️ Правила форматирования — крайне важно!】
═══════════════════════════════════════════════════════════════

【Один раздел = минимальная единица контента】
- Каждый раздел — минимальная единица отчёта
- ❌ Запрещены любые заголовки Markdown (#, ##, ###, ####)
- ❌ Не добавляйте заголовок раздела в начале
- ✅ Заголовок добавляется системой, пишите только текст
- ✅ Используйте **жирный**, абзацы, цитаты, списки — но не заголовки
...
【Рекомендации — используйте разные инструменты】
- insight_forge: глубокий анализ, автоматическая декомпозиция вопроса
- panorama_search: панорамный поиск, полная картина и хронология
- quick_search: быстрая проверка конкретного факта
- interview_agents: интервью с агентами, мнения от первого лица

═══════════════════════════════════════════════════════════════
【Рабочий процесс】
═══════════════════════════════════════════════════════════════

В каждом ответе вы можете сделать только одно из двух:

Вариант A — вызов инструмента:
Изложите мысли, затем вызовите инструмент:
<tool_call>
{{"name": "工具名称", "parameters": {{"参数名": "参数值"}}}}
</tool_call>
Система выполнит инструмент и вернёт результат. Не пишите результат сами.

Вариант B — финальный контент:
Когда информации достаточно, выведите содержание раздела, начав с "Final Answer:".

⚠️ Строго запрещено:
- Совмещать вызов инструмента и Final Answer в одном ответе
- Придумывать результаты инструментов (Observation) — их предоставляет система
- Вызывать более одного инструмента за раз

═══════════════════════════════════════════════════════════════
【Требования к содержанию раздела】
═══════════════════════════════════════════════════════════════

1. Содержание должно основываться на данных из инструментов
2. Обильно цитируйте оригинальные тексты
3. Используйте Markdown (но без заголовков):
   - **Жирный текст** для акцентов (вместо подзаголовков)
   - Списки (- или 1.2.3.) для структурирования
   - Пустые строки между абзацами
   - ❌ Запрещены #, ##, ###, #### и любые заголовки
4. 【Формат цитат — отдельным абзацем】
...
6. 【Избегайте повторов】Внимательно прочитайте уже написанные разделы
7. 【Ещё раз】Никаких заголовков! Используйте **жирный** вместо подзаголовков"""

SECTION_USER_PROMPT_TEMPLATE = """\
Уже написанные разделы (внимательно прочитайте, избегайте повторов):
{previous_content}
...
【Важные напоминания】
1. Внимательно прочитайте написанные разделы, не повторяйтесь!
2. Перед началом обязательно вызовите инструменты
3. Используйте разные инструменты
4. Содержание только из результатов поиска, не из собственных знаний

【⚠️ Предупреждение о формате — обязательно】
- ❌ Никаких заголовков (#, ##, ###, ####)
- ❌ Не начинайте с "{section_title}"
- ✅ Заголовок раздела добавляется системой
- ✅ Пишите текст, используйте **жирный** вместо подзаголовков

Начинайте:
1. Сначала подумайте (Thought), какая информация нужна
2. Затем вызовите инструмент (Action) для получения данных
3. Собрав достаточно информации, выведите Final Answer (только текст, без заголовков)"""
...
═══════════════════════════════════════════════════════════════
Вызвано инструментов {tool_calls_count}/{max_tool_calls} раз (использовано: {used_tools_str}）{unused_hint}
- Если информации достаточно: выведите содержание раздела, начав с "Final Answer:" (цитируйте тексты выше)
- Если нужно больше информации: вызовите инструмент
═══════════════════════════════════════════════════════════════"""

REACT_INSUFFICIENT_TOOLS_MSG = (
    "【Внимание】Вы вызвали инструменты {tool_calls_count} раз, минимум нужно {min_tool_calls}."
    "Вызовите ещё инструменты, затем выведите Final Answer.{unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Вызвано {tool_calls_count} инструментов, минимум нужно {min_tool_calls}."
    "Вызовите инструменты для получения данных.{unused_hint}"
)

REACT_TOOL_LIMIT_MSG = (
    "Достигнут лимит вызовов ({tool_calls_count}/{max_tool_calls}), больше нельзя."
    'Немедленно выведите содержание раздела, начав с "Final Answer:".'
)

REACT_UNUSED_TOOLS_HINT = "\n💡 Вы ещё не использовали: {unused_list}, попробуйте разные инструменты"

REACT_FORCE_FINAL_MSG = "Достигнут лимит вызовов, выведите Final Answer: с содержанием раздела."
...
Вы лаконичный и эффективный ассистент по прогнозированию.
...
【Правила】
1. Приоритетно отвечайте на основе отчёта выше
2. Отвечайте прямо, без длинных рассуждений
3. Используйте инструменты только если отчёта недостаточно
4. Ответы должны быть краткими, ясными, структурированными

【Инструменты】(только при необходимости, максимум 1-2 вызова)
{tools_description}
...
【Стиль ответа】
- Кратко и по делу
- Используйте > для цитирования ключевого контента
...
class ReportAgent:
    ...
    def plan_outline(
        ...
        Returns:
            ReportOutline: 报告大纲
        """
        logger.info("Планирование структуры отчёта...")
        ...
        try:
            ...
            logger.info(f"Структура готова: {len(sections)} разделов")
            return outline
            
        except Exception as e:
            logger.error(f"Ошибка планирования: {str(e)}")
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    messages.append({"role": "user", "content": "Продолжайте генерацию контента."})
                    continue
            ...
            if has_final_answer:
                ...
                logger.info(f"Раздел {section.title} завершён (вызовов инструментов: {tool_calls_count})")
        ...
        # 达到最大迭代次数，强制生成内容
        logger.warning(f"Раздел {section.title}: достигнут лимит итераций, принудительная генерация")
    ...
    def generate_report(
        ...
        try:
            ...
            ReportManager.update_progress(
                report_id, "planning", 5, "Планирование структуры отчёта...",
                completed_sections=[]
            )
            ...
            if progress_callback:
                progress_callback("planning", 0, "Планирование структуры отчёта...")
            ...
            ReportManager.update_progress(
                report_id, "planning", 15, f"Структура готова: {len(outline.sections)} разделов",
                completed_sections=[]
            )
            ReportManager.save_report(report)
            ...
            logger.info(f"Отчёт сгенерирован: {report_id}")
