...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "Вы эксперт по созданию профилей пользователей соцсетей. Генерируйте детальные, реалистичные профили для симуляции общественного мнения. Возвращайте валидный JSON, строковые значения без переносов строк. Используйте русский язык."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Сгенерируйте детальный профиль пользователя соцсетей для данной сущности, максимально приближенный к реальности.
...
1. bio: описание профиля в соцсети, 200 слов
2. persona: детальное описание персонажа (2000 слов, чистый текст), включая:
      - Базовая информация (возраст, профессия, образование, местоположение)
      - Предыстория (важный опыт, связь с событием, социальные связи)
      - Черты характера (MBTI, ключевые качества, способ выражения эмоций)
      - Поведение в соцсетях (частота постов, предпочтения контента, стиль общения)
      - Позиция и взгляды (отношение к теме, что может вызвать реакцию)
      - Уникальные черты (привычные выражения, особый опыт, хобби)
...
重要:
- Все значения — строки или числа, без переносов строк
- persona должен быть связным текстовым описанием
- Используйте русский язык (кроме gender — только английский male/female)
- Содержание должно соответствовать информации о сущности
- age — целое число, gender — "male" или "female"
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Сгенерируйте детальные настройки аккаунта соцсети для организации/группы, максимально приближенные к реальности.
...
1. bio: описание официального аккаунта, 200 слов, профессиональный стиль
2. persona: детальное описание аккаунта (2000 слов, чистый текст), включая:
      - Базовая информация (название, тип организации, история, функции)
      - Позиционирование аккаунта (тип, целевая аудитория, функции)
      - Стиль коммуникации (языковые особенности, типичные выражения, табу)
      - Особенности контента (типы, частота публикаций, активные часы)
      - Позиция (официальная позиция по ключевым темам, реакция на споры)
      - Особые указания (портрет представляемой группы, привычки ведения)
      - Память организации (связь с событием, действия и реакции организации)
3. age: фиксированно 30 (виртуальный возраст аккаунта)
4. gender: фиксированно "other" (для организаций)
5. mbti: тип MBTI для стиля аккаунта (например ISTJ — строгий и консервативный)
6. country: страна (на русском, например "Россия")
7. profession: описание функций организации
8. interested_topics: массив областей интересов

重要:
- Все значения — строки или числа, null запрещён
- persona должен быть связным текстовым описанием，不要使用换行符
- Используйте русский язык (кроме gender — только "other")
- age = 30, gender = "other"
- 机构账号发言要符合其身份定位"""
